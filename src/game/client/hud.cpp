	...
	if ( kv )
	{
		if ( kv->LoadFromFile( filesystem, "scripts/HudLayout.res" ) )
		{
			...
			for ( int i = 0; i < numelements; i++ )
			{
				...
				if ( !pPanel )
				{
					AssertMsg( 0, "Non-vgui hud element '%s'\n", m_HudList[i]->GetName() );
				...
				if ( !key )
				{
					AssertMsg( 0, "Hud element '%s' doesn't have an entry '%s' in scripts/HudLayout.res\n", m_HudList[i]->GetName(), pPanel->GetName() );
				...
				if ( !element->IsParentedToClientDLLRootPanel() && 
					 !pPanel->GetParent() )
				{
					AssertMsg( 0, "Hud element '%s'/'%s' doesn't have a parent\n", m_HudList[i]->GetName(), pPanel->GetName() );
